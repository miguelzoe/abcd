...
class MarketplaceAdminSummaryView(APIView):
    """
    GET /api/admin/marketplace/summary/
    Synthèse unifiée des indicateurs marketplace pour l'interface EFGH.
    """
    permission_classes = [IsAdministrator]

    def get(self, request):
        from django.db.models import Q
        from apps.marketplace.models import MarketplaceVehicle, MarketplaceOrder, Piece, Produit, MarketplaceArticleType, MarketplacePartnerApplication

        now = timezone.now()
        debut_mois = now.replace(day=1, hour=0, minute=0, second=0, microsecond=0)

        orders = MarketplaceOrder.objects.select_related('client__user', 'marketplace_user').all()
        vehicles = MarketplaceVehicle.objects.all()
        parts = Piece.objects.all()
        products = Produit.objects.all()
        article_types = MarketplaceArticleType.objects.all()
        partner_applications = MarketplacePartnerApplication.objects.select_related('user', 'reviewed_by').all()

        revenue_total = orders.filter(payment_status='paid').aggregate(total=Sum('total'))['total'] or 0
        revenue_month = orders.filter(payment_status='paid', created_at__gte=debut_mois).aggregate(total=Sum('total'))['total'] or 0
        ...
        type_counts = {
            ...
            for item in orders.values('type').annotate(count=Count('id'))
        }

        low_stock = parts.filter(stock__lte=5).count()
        pending_orders = orders.filter(status='pending').count()
        active_vehicles = vehicles.filter(is_active=True).count()
        featured_vehicles = vehicles.filter(is_featured=True, is_active=True).count()

        latest_orders = []
        for order in orders.order_by('-created_at')[:8]:
            user = order.marketplace_user or (order.client.user if order.client else None)
            latest_orders.append({
                'id': order.id,
                'type': order.type,
                'status': order.status,
                'payment_status': order.payment_status,
                'client_name': (user.get_full_name() or user.username) if user else 'Client supprimé',
                'total': order.total,
                'created_at': order.created_at,
            })

        latest_partner_applications = []
        for app in partner_applications.order_by('-created_at')[:8]:
            latest_partner_applications.append({
                'id': app.id,
        ...
        return Response({
            ...
            'orders_month': orders.filter(created_at__gte=debut_mois).count(),
            'vehicles_total': vehicles.count(),
            'vehicles_active': active_vehicles,
            'vehicles_featured': featured_vehicles,
            'parts_total': parts.count(),
            'products_total': products.count(),
            'article_types_total': article_types.count(),
            'partner_applications_total': partner_applications.count(),
            'partner_applications_pending': partner_applications.filter(status='pending').count(),
            'partners_total': User.objects.filter(user_type__in=['vendor', 'auto_shop']).count(),
            'partners_approved': Vendor.objects.filter(approval_status='approved').count(),
            'vendors_total': User.objects.filter(user_type='vendor').count(),
            'auto_shops_total': User.objects.filter(user_type='auto_shop').count(),
            ...
            'latest_partner_applications': latest_partner_applications,
        })


class MarketplacePartnerApplicationsAdminView(APIView):
    """Liste des demandes d’accréditation marketplace côté administration."""
    permission_classes = [IsAdministrator]

    def get(self, request):
        from apps.marketplace.models import MarketplacePartnerApplication
        from apps.marketplace.serializers import MarketplacePartnerApplicationSerializer
        qs = MarketplacePartnerApplication.objects.select_related('user', 'reviewed_by').all()
        status_filter = request.query_params.get('status')
        partner_type = request.query_params.get('partner_type') or request.query_params.get('type')
        search = request.query_params.get('search', '').strip()
        if status_filter:
        ...
        if search:
            qs = qs.filter(Q(company_name__icontains=search) | Q(user__email__icontains=search) | Q(user__telephone__icontains=search) | Q(user__first_name__icontains=search) | Q(user__last_name__icontains=search))
        return Response(MarketplacePartnerApplicationSerializer(qs.order_by('-created_at'), many=True).data)


class MarketplacePartnerApplicationApproveView(APIView):
    """Valide une demande et transforme le compte marketplace en vendeur/boutique."""
    permission_classes = [IsAdministrator]

    def post(self, request, pk):
        from apps.marketplace.models import MarketplacePartnerApplication
        from apps.marketplace.serializers import MarketplacePartnerApplicationSerializer
        application = get_object_or_404(MarketplacePartnerApplication, pk=pk)
        application.approve(admin_user=request.user)
        return Response(MarketplacePartnerApplicationSerializer(application).data)


class MarketplacePartnerApplicationRejectView(APIView):
    """Refuse une demande d’accréditation marketplace."""
    permission_classes = [IsAdministrator]

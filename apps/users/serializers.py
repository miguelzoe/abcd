...
class TechnicianRegisterSerializer(serializers.Serializer):
    ...
    def _parse_specialties(self, raw):
        ...
        if isinstance(raw, (list, tuple)):
            items = []
            for item in raw:
                items.extend(self._parse_specialties(item))
            return items
...
class VendorSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Vendor
        fields = [
            'id', 'user', 'company_name', 'business_license', 'partner_type',
            'approval_status', 'rejection_reason', 'approved_at', 'approved_by',
            'rating', 'total_sales', 'is_verified', 'total_produits'
        ]
        read_only_fields = ['approval_status', 'rejection_reason', 'approved_at', 'approved_by']
...
class VendorUpdateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Vendor
        fields = ['company_name', 'business_license', 'partner_type']

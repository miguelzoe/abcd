...
class InterventionForgotPasswordView(APIView):
    """Réinitialisation mobile intervention-cartronic : clients et techniciens uniquement."""
    permission_classes = [permissions.AllowAny]
    throttle_classes = [AuthRateThrottle]
    ...
    def post(self, request):
        ...
        user = User.objects.filter(
            Q(email__iexact=identifier) | Q(username__iexact=identifier) | Q(telephone=identifier),
            user_type__in=['client', 'technician'],
            is_active=True,
        ).first()
        payload = {'message': "Si ce compte intervention existe, un lien de réinitialisation a été préparé.", 'status': 'accepted'}
        if not user or not user.email:
            return Response(payload, status=status.HTTP_200_OK)
        ...
        frontend_base = (getattr(settings, 'INTERVENTION_FRONTEND_URL', '') or 'cartronic://forgot-password').rstrip('/')
        separator = '&' if '?' in frontend_base else '?'
        reset_url = f"{frontend_base}{separator}token={quote(reset_token, safe='')}"

        try:
            send_mail(
                subject='Réinitialisation de votre mot de passe Intervention Cartronic',
                message=(
                    "Bonjour,\n\n"
                    "Vous avez demandé à réinitialiser votre mot de passe sur l’application Intervention Cartronic.\n"
                    f"Lien : {reset_url}\n"
                    f"Code : {reset_token}\n\n"
            ...
            pass
        if getattr(settings, 'INTERVENTION_PASSWORD_RESET_EXPOSE_TOKEN', False):
            payload.update({'reset_token': reset_token, 'reset_url': reset_url})
        return Response(payload, status=status.HTTP_200_OK)


class InterventionResetPasswordView(APIView):
    """Confirmation de réinitialisation mobile intervention-cartronic."""
    permission_classes = [permissions.AllowAny]
    throttle_classes = [AuthRateThrottle]
    ...
    def post(self, request):
        ...
        try:
            uidb64, token = token_str.split(':', 1)
            uid = force_str(urlsafe_base64_decode(uidb64))
            user = User.objects.get(pk=uid, user_type__in=['client', 'technician'], is_active=True)
        except (ValueError, TypeError, User.DoesNotExist, DjangoUnicodeDecodeError):
            return Response({'message': "Ce lien de réinitialisation est invalide ou a expiré."}, status=status.HTTP_400_BAD_REQUEST)
        if not default_token_generator.check_token(user, token):
            return Response({'message': "Ce lien de réinitialisation est invalide ou a expiré."}, status=status.HTTP_400_BAD_REQUEST)
        user.set_password(serializer.validated_data['new_password'])
        if hasattr(user, 'password_changed_at'):
            ...
            user.save(update_fields=['password'])
        return Response({'message': 'Mot de passe intervention réinitialisé avec succès', 'status': 'success'})

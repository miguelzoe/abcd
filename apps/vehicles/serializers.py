...
# Les véhicules ajoutés par les clients doivent provenir du référentiel tarifaire.
# Ainsi, la réservation et le moteur de tarification parlent exactement le même langage.
def _catalog_vehicle_qs(brand: str, model: str):
    return VehicleCatalog.objects.filter(
...
def _vehicle_type_from_gamme(gamme: str) -> str:
    if gamme == 'suv':
        return 'suv'
    if gamme == '4x4':
        return 'pickup'
...
def _normalize_vehicle_from_catalog(attrs: dict, *, partial: bool = False) -> dict:
    brand = attrs.get('brand')
    model = attrs.get('model')
    if partial and brand is None and model is None:
        return attrs

    # En modification partielle, compléter avec l'objet courant si nécessaire.
    instance = attrs.pop('_instance_for_catalog', None)
    if instance is not None:
        brand = brand if brand is not None else instance.brand
        model = model if model is not None else instance.model

    if not brand or not model:
        raise serializers.ValidationError({
            'vehicle_catalog': 'Choisissez un véhicule dans le catalogue Cartronic.'
        })

    item = _catalog_vehicle_qs(brand, model).first()
    if not item:
        raise serializers.ValidationError({
            'vehicle_catalog': 'Ce modèle n’est pas dans le référentiel Cartronic actif. Choisissez un modèle proposé dans la liste.'
        })

    attrs['brand'] = item.brand
    attrs['model'] = item.model
    # Le type est automatiquement cohérent avec la gamme tarifaire, sauf si une valeur explicite est déjà acceptée.
    attrs['vehicle_type'] = attrs.get('vehicle_type') or _vehicle_type_from_gamme(item.gamme)
    return attrs

class VehicleSerializer(serializers.ModelSerializer):
    """Serializer de base pour les véhicules"""
    type_vehicule_display = serializers.CharField(source='get_vehicle_type_display', read_only=True)
...
class VehicleCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        return _normalize_vehicle_from_catalog(attrs, partial=False)


class VehicleUpdateSerializer(serializers.ModelSerializer):
    """Serializer pour modifier un véhicule client.

    La marque/le modèle peuvent changer, mais uniquement vers un modèle présent dans le
    référentiel tarifaire actif. Les champs FR et EN sont acceptés pour compatibilité mobile.
    """
    ...
    class Meta:
        ...
        fields = [
            'marque', 'modele', 'type_vehicule', 'annee', 'couleur',
            'numero_immatriculation', 'numero_chassis', 'kilometrage', 'statut', 'notes',
            'brand', 'model', 'vehicle_type', 'year', 'color', 'license_plate', 'vin', 'mileage',
        ]

    def to_internal_value(self, data):
        d = dict(data)
        mapping = {
            'brand': 'marque', 'model': 'modele', 'vehicle_type': 'type_vehicule',
            'year': 'annee', 'color': 'couleur', 'license_plate': 'numero_immatriculation',
            'vin': 'numero_chassis', 'mileage': 'kilometrage',
        }
        for en, fr in mapping.items():
            if en in d and fr not in d:
                d[fr] = d.get(en)
        for key in ('numero_immatriculation', 'numero_chassis'):
            if d.get(key) == '':
                d[key] = None
    ...
    def validate_annee(self, value):
        ...
        if value < 1900 or value > current_year + 1:
            raise serializers.ValidationError(f"L'année doit être comprise entre 1900 et {current_year + 1}")
        return value

    def validate(self, attrs):
        attrs['_instance_for_catalog'] = self.instance
        return _normalize_vehicle_from_catalog(attrs, partial=True)
...
class VehicleCatalogSerializer(serializers.ModelSerializer):
    coefficient = serializers.DecimalField(max_digits=4, decimal_places=2, required=False)

    class Meta:
        model = VehicleCatalog
        fields = ['id', 'brand', 'model', 'gamme', 'coefficient', 'is_active', 'created_at', 'updated_at']
        read_only_fields = ['id', 'created_at', 'updated_at']

    def to_internal_value(self, data):
        d = dict(data)
        # L'admin web français saisit souvent 1,3 au lieu de 1.3.
        if isinstance(d.get('coefficient'), str):
            d['coefficient'] = d['coefficient'].replace(',', '.')
        return super().to_internal_value(d)

    def validate(self, attrs):
        if not attrs.get('coefficient'):
            gamme = attrs.get('gamme') or getattr(self.instance, 'gamme', 'standard')
            defaults = {
                'economique': '1.00', 'standard': '1.30', 'suv': '1.60',
                '4x4': '1.80', 'utilitaire': '2.00', 'premium': '2.50',
            }
            attrs['coefficient'] = defaults.get(gamme, '1.30')
        return attrs

...
class VehicleCatalog(models.Model):
    """Référentiel administrable des véhicules utilisés par le moteur de tarification."""
    GAMME_CHOICES = (
    ...
    gamme = models.CharField(max_length=30, choices=GAMME_CHOICES, default='standard')
    coefficient = models.DecimalField(max_digits=4, decimal_places=2, default=1.30)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['brand', 'model']),
            models.Index(fields=['gamme', 'is_active']),
        ]

    def __str__(self):
        return f'{self.brand} {self.model} ({self.gamme})'

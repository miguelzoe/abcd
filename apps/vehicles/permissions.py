...
class IsVehicleOwner(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        ...
        # Compatibilité: certains véhicules anciens ne sont liés qu'en FK `client`,
        # d'autres via la relation M2M `proprietaires`.
        if request.user.user_type == 'client':
            if getattr(getattr(obj, 'client', None), 'user_id', None) == request.user.id:
                return True

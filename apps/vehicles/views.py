...
from apps.vehicles.serializers import (
    ...
    MaintenanceRecordCreateSerializer, MaintenanceStatsSerializer, VehicleCatalogSerializer
)
...
class VehicleCatalogViewSet(viewsets.ModelViewSet):
    """Référentiel véhicule consultable par le mobile et administrable par l’admin."""
    ...
    def perform_create(self, serializer):
        user = self.request.user
        if getattr(user, 'user_type', None) != 'administrator' and not user.is_staff:
            raise PermissionDenied("Seul un administrateur peut ajouter un type de véhicule")
        serializer.save()

    def perform_update(self, serializer):
        user = self.request.user
        if getattr(user, 'user_type', None) != 'administrator' and not user.is_staff:
            raise PermissionDenied("Seul un administrateur peut modifier un type de véhicule")
        serializer.save()
